document.addEventListener("DOMContentLoaded", function () {
  ...
  function fetchFilteredBooks() {
    ...
    const selectedAuthors = Array.from(
      document.querySelectorAll('input[name="author_filter[]"]:checked')
    ).map((cb) => cb.value)
    const selectedGenres = Array.from(
      document.querySelectorAll('input[name="genre_filter[]"]:checked')
    ).map((cb) => cb.value)
    ...
    selectedAuthors.forEach((author) =>
      queryParams.append("author_filter[]", author)
    )
    selectedGenres.forEach((genre) =>
      queryParams.append("genre_filter[]", genre)
    )
    ...
    fetch(ajaxUrl)
      ...
      .catch((error) => {
        ...
        bookListDiv.innerHTML =
          '<p class="no-books-message-ajax">Error loading books. Please try again.</p>'
      })
  }
  ...
  if (resetFiltersButton) {
    resetFiltersButton.addEventListener("click", function () {
      document
        .querySelectorAll('input[name="author_filter[]"]')
        .forEach((cb) => (cb.checked = false))
      document
        .querySelectorAll('input[name="genre_filter[]"]')
        .forEach((cb) => (cb.checked = false))
      fetchFilteredBooks()
    })
  }
